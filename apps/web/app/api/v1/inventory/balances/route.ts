...
import { inventoryToCsv, listInventoryBalances } from "@crushermitra/database";
...
export async function GET(request: Request) {
  ...
  try {
    const result = await listInventoryBalances(toTenantContext(session), parsedQuery.data);
    if (parsedQuery.data.export === "csv") {
      return new Response(inventoryToCsv(result), {
        headers: {
          "content-disposition": 'attachment; filename="inventory-balances.csv"',
    ...
    return jsonError("Unable to load inventory balances", 500);
  }
}
