...
import { z } from "zod";
import { rejectInventoryCorrection } from "@crushermitra/database";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (!parsedId.success || !parsedBody.success) {
    return jsonError("Invalid rejection request", 400);
  }

  try {
    return NextResponse.json(await rejectInventoryCorrection(toTenantContext(session), parsedId.data, parsedBody.data.reason));
  } catch (error) {
    if (isDatabaseUnavailable(error)) {
      return jsonError(databaseUnavailableMessage, 503);
    }
    return jsonError(error instanceof Error ? error.message : "Unable to reject inventory correction", 400);
  }
}
