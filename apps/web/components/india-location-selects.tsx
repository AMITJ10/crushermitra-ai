export const defaultIndianState = "Maharashtra";
export const defaultMaharashtraDistrict = "Pune";
...
export const districtsByState: Record<string, string[]> = {
  ...
  Chhattisgarh: ["Balod", "Baloda Bazar", "Balrampur", "Bastar", "Bemetara", "Bijapur", "Bilaspur", "Dantewada", "Dhamtari", "Durg", "Gariaband", "Gaurela-Pendra-Marwahi", "Janjgir-Champa", "Jashpur", "Kabirdham", "Kanker", "Kondagaon", "Korba", "Korea", "Mahasamund", "Mungeli", "Narayanpur", "Raigarh", "Raipur", "Rajnandgaon", "Sakti", "Sarangarh-Bilaigarh", "Sukma", "Surajpur", "Surguja"],
...
export const defaultPinCodeByDistrict: Record<string, string> = {
  Pune: "411001",
  "Mumbai City": "400001",
  ...
  Nanded: "431601",
  Jalgaon: "425001",
  "Aurangabad/Chhatrapati Sambhajinagar": "431001",
  "Ahmednagar/Ahilyanagar": "414001"
};
...
export function StateSelect({ id, name, onChange, required, value }: SelectProps) {
  return (
    <select
      ...
      required={required}
      value={value || defaultIndianState}
    >
...
export function DistrictSelect({ id, name, onChange, required, state, value }: SelectProps & { state: string }) {
  const districts = districtsByState[state || defaultIndianState] ?? [];
  const selectedValue = value && districts.includes(value) ? value : districts[0] ?? "";
...
export function PinCodeInput({ id, name, onChange, required, value }: SelectProps) {
  return (
    <input
      className="min-h-11 rounded-md border border-slate-300 px-3 text-slate-950 outline-none focus:border-cyan-700 focus:ring-2 focus:ring-cyan-100"
      id={id}
